...
int main()
{
    ...
    // Kernel launch. Naive 3D grid, one thread per output voxel, global
    // memory only -- baseline; tiled / shared-memory variants are follow-ups.
    dim3 block(8, 8, 8);
    dim3 grid((XDIM + block.x - 1) / block.x,
